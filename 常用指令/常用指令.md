...
$$\
J(h) = \int \hat{p}^2\left( x; \mathbf{x}^{(n)} \right) dx - 2 \int \hat{p} \left( x; \mathbf{x}^{(n)} \right) p(x) dx
\hat{J}(h) = \int \hat{p}^2 \left( x; \mathbf{x}^{(n)} \right) dx - \frac{2}{n}\sum_{i = 1}^n \hat{p} \left( \mathbf{X}_i ; \mathbf{x}^{(n\backslash i)} \right), \tag{1}
\hat{J}(h) = \frac{2}{(n - 1)h} - \frac{n+1}{(n-1)h}\sum_{i = 1}^m \hat{p}_j^2, \quad \hat{p}_j = \frac{n_j}{n}. \quad (2)\]$$

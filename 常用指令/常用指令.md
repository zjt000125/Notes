...
conda create -n slot_attention python=3.8

conda create -n slot_attention python=3.9
...
conda remove -n slot_attention --all

虚拟环境改名

conda create --name nerf_studio --clone nerfstudio
conda remove --name nerfstudio --all
...
```bash
sudo ln -s /home/jiantong/local/miniconda3/envs/posediffusion/lib/libopenh264.so.5 /usr/lib/libopenh264.so.5
```
